...
async def scheduler_jobs():
    ...
    scheduler.add_job(del_order_day, "cron", day='*', hour=0, minute=0, timezone=timezone) #Каждый день в 00:00
    scheduler.add_job(fake_game_adm, 'interval', seconds=TIMER, timezone=timezone)
    scheduler.add_job(warning_check_day, "cron", day='*', hour=23, minute=55, timezone=timezone) #Каждый день в 00:00 удаляются все чеки
